...
def _decision_to_dict(decision: GateDecision) -> dict[str, Any]:
    return {
        "allowed_units": [_unit_to_dict(unit) for unit in decision.allowed_units],
        "dropped_units": [_unit_to_dict(unit) for unit in decision.dropped_units],
...
def _decision_from_dict(data: dict[str, Any]) -> GateDecision:
    return GateDecision(
        allowed_units=[_unit_from_dict(item) for item in data.get("allowed_units", [])],
        dropped_units=[_unit_from_dict(item) for item in data.get("dropped_units", [])],
...
def _enforcement_to_dict(result: EnforcementResult) -> dict[str, Any]:
    return {
        ...
        "kept_units": [_unit_to_dict(unit) for unit in result.kept_units],
        "dropped_units": [_unit_to_dict(unit) for unit in result.dropped_units],
...
def _enforcement_from_dict(data: dict[str, Any]) -> EnforcementResult:
    return EnforcementResult(
        ...
        kept_units=[_unit_from_dict(item) for item in data.get("kept_units", [])],
        dropped_units=[_unit_from_dict(item) for item in data.get("dropped_units", [])],
...
def to_json(obj: SerializablePayload) -> str:
    ...
    payload: dict[str, Any] = {"ega_schema_version": EGA_SCHEMA_VERSION}
...
def from_json(payload: str, target_type: SerializableType) -> SerializablePayload:
    ...
    version = str(decoded.get("ega_schema_version"))
